...
class ValidatorFactory {
    static create(spec, options) {
        const type = options.projectType ?? exports.ProjectType.SME;
        switch (type) {
    ...
    static specFilter(filter, unResolveSpec, resolvedSpec, options) {
        try {
            ...
            for (const filterItem of filter) {
                ...
                const pathObj = resolvedSpec.paths?.[path];
                if (ConstantString.AllOperationMethods.includes(methodName) &&
                    ...
                    if (!newPaths[path][methodName].operationId) {
                        newPaths[path][methodName].operationId =
...
class ManifestUpdater {
    static async useCopilotExtensionsInSchema(manifest) {
        const schema = await appManifest.AppManifestUtils.fetchSchema(manifest.$schema);
    ...
    static updateManifestDescription(manifest, spec) {
        manifest.description = {
            ...
            full: (spec.info.description ?? manifest.description.full)?.slice(0, ConstantString.FullDescriptionMaxLens),
        };
    }
    ...
    static async generatePluginManifestSchema(spec, specRelativePath, apiPluginFilePath, appName, authMap, options, existingPluginManifestInfo) {
                ...
                for (const method in operations) {
                    if (options.allowMethods.includes(method)) {
                        ...
                        if (operationItem) {
                            ...
                            const conversationStarterStr = (summary ?? description).slice(0, ConstantString.ConversationStarterMaxLens);
        ...
        let apiPlugin;
        if (await fs__default['default'].pathExists(apiPluginFilePath)) {
            apiPlugin = await fs__default['default'].readJSON(apiPluginFilePath);
        }
        else if (existingPluginManifestInfo &&
            (await fs__default['default'].pathExists(existingPluginManifestInfo.manifestPath))) {
            apiPlugin = await fs__default['default'].readJSON(existingPluginManifestInfo.manifestPath);
        }
        else {
            apiPlugin = {
    ...
    static async updateManifest(manifestPath, outputSpecPath, spec, options, adaptiveCardFolder, authInfo) {
        try {
            const originalManifest = await fs__default['default'].readJSON(manifestPath);
            const updatedPart = {};
                        ...
                        if (options.allowMethods?.includes(method)) {
                            ...
                            if (operationItem) {
                                ...
                                if (adaptiveCardFolder) {
                                    ...
                                    command.apiResponseRenderingTemplateFile = (await fs__default['default'].pathExists(adaptiveCardPath))
...
class SpecParser {
    ...
    async list() {
        try {
            ...
            for (const apiKey in apiMap) {
                ...
                const operationId = operation.operationId ?? `${method.toLowerCase()}${Utils.convertPathToCamelCase(path)}`;
                const apiResult = {
                    api: apiKey,
                    server: "",
    ...
    async generateForCopilot(manifestPath, filter, outputSpecPath, pluginFilePath, existingPluginFilePath, signal, adaptiveCardUpdateStrategy) {
        ...
        try {
            ...
            await fs__default['default'].outputJSON(manifestPath, updatedManifest, { spaces: 4 });
            await fs__default['default'].outputJSON(pluginFilePath, apiPlugin, { spaces: 4 });
        }
        catch (err) {
            if (err instanceof SpecParserError) {
                throw err;
    ...
    async generate(manifestPath, filter, outputSpecPath, adaptiveCardFolder, signal) {
        ...
        try {
                    ...
                    for (const method in newSpec.paths[url]) {
                        ...
                        if (this.options.allowMethods.includes(method)) {
                            ...
                            try {
                                ...
                                await fs__default['default'].outputJSON(fileName, wrappedCard, { spaces: 2 });
                                const dataFileName = path__default['default'].join(adaptiveCardFolder, `${safeAdaptiveCardName}.data.json`);
                                await fs__default['default'].outputJSON(dataFileName, jsonData, { spaces: 2 });
                            }
                            catch (err) {
            ...
            await fs__default['default'].outputJSON(manifestPath, updatedManifest, { spaces: 2 });
            result.warnings.push(...warnings);
        }
        catch (err) {
            if (err instanceof SpecParserError) {
    ...
    async generateAdaptiveCardInPlugin(pluginFilePath, filter, signal) {
        ...
        const newSpecs = await this.getFilteredSpecs(filter, signal);
        const newSpec = newSpecs[1];
        const apiPlugin = (await fs__default['default'].readJSON(pluginFilePath));
        const jsonDataSet = {};
            ...
            for (const func of functions) {
                if (func.capabilities?.response_semantics) {
                    ...
                    if (card && Object.keys(card).length !== 0) {
                        ...
                        responseSemantic.static_template = { file: `adaptiveCards/${cardName}.json` };
                        await fs__default['default'].outputJSON(cardPath, card, { spaces: 4 });
                        const data = jsonDataSet[cardName] ?? {};
                        await fs__default['default'].outputJSON(dataPath, data, { spaces: 4 });
                    }
                }
            }
        }
    ...
    async loadSpec() {
        if (!this.spec) {
            ...
            if (!this.unResolveSpec.openapi && this.unResolveSpec.swagger === "2.0") {
                const specObj = await converter__default['default'].convert(this.unResolveSpec, {});
    ...
    async saveFilterSpec(outputSpecPath, unResolvedSpec) {
        ...
        await fs__default['default'].outputFile(outputSpecPath, resultStr);
    }

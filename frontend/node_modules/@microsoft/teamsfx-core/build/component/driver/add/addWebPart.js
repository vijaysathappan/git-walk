...
const constants_2 = require("../../generator/spfx/utils/constants");
...
let AddWebPartDriver = class AddWebPartDriver {
    description = (0, localizeUtils_1.getLocalizedString)("driver.spfx.add.description");
    async run(args, context) {
        const wrapContext = new wrapUtil_1.WrapDriverContext(context, constants_3.Constants.ActionName, constants_3.Constants.ActionName);
        return (0, common_1.wrapRun)(() => this.add(args, wrapContext), constants_3.Constants.ActionName);
    }
    ...
    async add(args, context) {
        ...
        const yorcPath = path_1.default.join(spfxFolder, constants_3.Constants.YO_RC_FILE);
        ...
        inputs[constants_1.QuestionNames.SPFxWebpartName] = webpartName;
        inputs[constants_1.QuestionNames.SPFxFramework] = framework;
        inputs[constants_1.QuestionNames.SPFxFolder] = spfxFolder;
        inputs[constants_1.QuestionNames.ManifestPath] = manifestPath;
        inputs[constants_1.QuestionNames.LocalTeamsAppManifestFilePath] = localManifestPath;
        inputs[constants_1.QuestionNames.SPFxInstallPackage] = spfxPackage;
        ...
        const SPFxContext = (0, globalVars_1.createContext)();
        const yeomanRes = await spfxGenerator_1.SPFxGenerator.doYeomanScaffold(SPFxContext, inputs, context.projectPath);
        if (yeomanRes.isErr())
            throw yeomanRes.error;
        context.logProvider.verbose(`Succeeded to add web part '${webpartName}'.`);
        const componentId = yeomanRes.value;
        const useNewDevUrl = SPFxContext &&
            SPFxContext.templateVariables &&
            SPFxContext.templateVariables["useNewDevUrl"] == "true";
        const remoteStaticSnippet = {
            entityId: componentId,
            name: webpartName,
            contentUrl: util.format(constants_3.Constants.REMOTE_CONTENT_URL, componentId),
        ...
        const localStaticSnippet = {
            entityId: componentId,
            name: webpartName,
            contentUrl: util.format(useNewDevUrl ? constants_3.Constants.LOCAL_CONTENT_URL_NEW : constants_3.Constants.LOCAL_CONTENT_URL, componentId),
            websiteUrl: constants_2.ManifestTemplate.WEBSITE_URL,
            scopes: ["personal"],
        };
        ...
        const localRes = await ManifestUtils_1.manifestUtils.addCapabilities.bind(ManifestUtils_1.manifestUtils)({ ...inputs, projectPath: context.projectPath }, [{ name: "staticTab", snippet: localStaticSnippet }]);
        if (localRes.isErr())
        ...
        const remoteRes = await ManifestUtils_1.manifestUtils.addCapabilities.bind(ManifestUtils_1.manifestUtils)({ ...inputs, projectPath: context.projectPath }, [{ name: "staticTab", snippet: remoteStaticSnippet }]);
        if (remoteRes.isErr())
            throw remoteRes.error;
        context.logProvider.verbose(`Succeeded to update remote manifest file.`);
        context.ui?.showMessage("info", (0, localizeUtils_1.getLocalizedString)("driver.spfx.add.successNotice", webpartName), false);
        return new Map();
    }
};
exports.AddWebPartDriver = AddWebPartDriver;
tslib_1.__decorate([
    (0, lib_1.hooks)([(0, addStartAndEndTelemetry_1.addStartAndEndTelemetry)(constants_3.Constants.ActionName, constants_3.Constants.ActionName)]),
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", [Object, Object]),
    tslib_1.__metadata("design:returntype", Promise)
], AddWebPartDriver.prototype, "run", null);
exports.AddWebPartDriver = AddWebPartDriver = tslib_1.__decorate([
    (0, typedi_1.Service)(constants_3.Constants.ActionName)
], AddWebPartDriver);

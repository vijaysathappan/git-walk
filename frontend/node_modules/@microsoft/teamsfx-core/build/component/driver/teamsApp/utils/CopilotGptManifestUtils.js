...
class CopilotGptManifestUtils {
    ...
    async validateAgainstSchema(declaraitveCopilot, manifestPath, context) {
        ...
        try {
            const manifestValidationRes = await teamsfx_api_1.AppManifestUtils.validateAgainstSchema(manifestRes.value);
            const res = {
            ...
            if (skills && Array.isArray(skills)) {
                for (const skill of skills) {
                    ...
                    const skillFolderPath = path_1.default.join(path_1.default.dirname(manifestPath), skill.folder);
                    if (!(await fs_extra_1.default.pathExists(skillFolderPath))) {
                        skillResult.validationResult.push(`Skill folder not found: ${skillFolderPath}`);
                    }
                    else {
                        const skillMdPath = path_1.default.join(skillFolderPath, "SKILL.md");
                        skillResult.filePath = skillMdPath;
                        if (!(await fs_extra_1.default.pathExists(skillMdPath))) {
                            skillResult.validationResult.push(`SKILL.md not found in skill folder: ${skillFolderPath}`);
                        }
                        else {
                            const content = await fs_extra_1.default.readFile(skillMdPath, { encoding: "utf-8" });
                            const frontmatter = this.parseYamlFrontmatter(content);
                            if (!frontmatter.name) {
                                skillResult.validationResult.push(`SKILL.md is missing required field 'name' in YAML frontmatter: ${skillMdPath}`);
                            }
                            if (!frontmatter.description) {
                                skillResult.validationResult.push(`SKILL.md is missing required field 'description' in YAML frontmatter: ${skillMdPath}`);
                            }
                        }
                    }
    ...
    async updateConversationStarters(actionPath, gptManifest) {
        ...
        if (!conversationStarters || conversationStarters.length === 0) {
            const openApiRuntimes = actionManifest.runtimes?.filter((runtime) => runtime.type === "OpenApi");
            if (openApiRuntimes) {
                for (const runtime of openApiRuntimes) {
                    ...
                    if (await fs_extra_1.default.pathExists(specPath)) {
                        ...
                        const operationIds = actionManifest.functions?.map((func) => func.name);
    ...
    async addSkill(copilotGptPath, folder) {
        const gptManifestRes = await exports.copilotGptManifestUtils.readCopilotGptManifestFile(copilotGptPath);
        if (gptManifestRes.isErr()) {
            return (0, teamsfx_api_1.err)(gptManifestRes.error);
        }
        ...
        const updateRes = await exports.copilotGptManifestUtils.writeCopilotGptManifestFile(gptManifest, copilotGptPath);
        if (updateRes.isErr()) {
        ...
        if (platform !== teamsfx_api_1.Platform.CLI) {
            ...
            for (const skillValidationRes of validationRes.skillValidationResult ?? []) {
                if (skillValidationRes.validationResult.length > 0) {
                    const skillErrors = skillValidationRes.validationResult
                        .map((error) => `${constant_1.SummaryConstant.Failed} ${error}`)
                        .join(os_1.EOL);
                    const skillPath = skillValidationRes.filePath || skillValidationRes.folder;
                    outputMessage +=
                        (!outputMessage ? "" : os_1.EOL) + `Skill validation (${skillPath}):` + os_1.EOL + skillErrors;
                }
            }
            ...
            for (const skillValidationRes of validationRes.skillValidationResult ?? []) {
                if (skillValidationRes.validationResult.length > 0) {
                    ...
                    skillValidationRes.validationResult.map((error) => {
                        outputMessage.push({
                            content: `${constant_1.SummaryConstant.Failed} `,
    ...
    async addOrUpdateCapability(agentManifestPath, capabilityName, manifestRes, capabilityData) {
        ...
        const newCapability = { name: capabilityName, ...capabilityData };

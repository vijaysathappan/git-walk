...
const daSpecParser = tslib_1.__importStar(require("../../../common/daSpecParser"));
const kiotaClient = tslib_1.__importStar(require("../../../common/kiotaClient"));
const localizeUtils_1 = require("../../../common/localizeUtils");
const versionMetadata_1 = require("../../../common/versionMetadata");
const common_1 = require("../../../error/common");
const openApiSpecHelper = tslib_1.__importStar(require("../../generator/openApiSpec/helper"));
...
const typeSpecCompileError_1 = require("./error/typeSpecCompileError");
const actionName = "typeSpec/compile"; // DO NOT MODIFY the name
let TypeSpecCompileDriver = class TypeSpecCompileDriver {
    description = (0, localizeUtils_1.getLocalizedString)("driver.typeSpec.compile.description");
    progressTitle = (0, localizeUtils_1.getLocalizedString)("driver.typeSpec.progressBar");
    async execute(args, ctx, outputEnvVarNames) {
        const summaries = [];
        const outputs = new Map();
        try {
            ctx.logProvider?.info((0, localizeUtils_1.getLocalizedString)("driver.typeSpec.compile.start", actionName));
            ...
            if (ctx.ui?.runCommand) {
                ...
                if (!fs_extra_1.default.existsSync(openApiSpecsFolderPath)) {
                    const tspOutput = tspRes.value;
                    if (tspOutput) {
                        ctx.logProvider?.error(tspOutput);
                    }
                    throw new typeSpecCompileError_1.TypeSpecCompileError(actionName, tspOutput);
                }
                ...
                if (actions && actions.length > 0) {
                    if (openapiSpecs.length === 1) {
                        ...
                        await kiotaClient.kiotageneratePlugin(`${openApiSpecsFolderPath}/${spec}`, `${outputFolderPath}`, `${pluginManifestName}`, `${outputFolderPath}`, undefined, undefined, undefined, undefined, true);
                        await daSpecParser.patchOpenApiExtensionsIntoPluginManifest(`${openApiSpecsFolderPath}/${spec}`, path_1.default.join(outputFolderPath, `${pluginManifestName.toLowerCase()}-apiplugin.json`));
                    }
                    else {
                ...
                for (const file of generatedFolder) {
                    if (file.match(/[^-]+\-apiplugin\.json/)) {
                        ...
                        const authData = await daSpecParser.parseAndUpdatePluginManifestForKiota(pluginManifestPath, true);
                        for (const authInfo of authData) {
                            const addAuthRes = await openApiSpecHelper.injectAuthAction(ctx.projectPath, authInfo.authName, undefined, path_1.default.join(outputFolderPath, authInfo.specPath), false, authInfo.authType === "apiKey" ? "ApiKeyPluginVault" : "OAuthPluginVault", false, authInfo.registrationId);
                            if (addAuthRes) {
                                showAlert = true;
                            }
                        }
            ...
            ctx.logProvider?.info((0, localizeUtils_1.getLocalizedString)("driver.aadApp.log.successExecuteDriver", actionName));
            return {
            ...
            if (error instanceof teamsfx_api_1.UserError || error instanceof teamsfx_api_1.SystemError) {
                ctx.logProvider?.error((0, localizeUtils_1.getLocalizedString)("driver.aadApp.log.failExecuteDriver", actionName, error.displayMessage));
                return {
            ...
            ctx.logProvider?.error((0, localizeUtils_1.getLocalizedString)("driver.aadApp.log.failExecuteDriver", actionName, message));
            return {
...
exports.TypeSpecCompileDriver = TypeSpecCompileDriver = tslib_1.__decorate([

...
exports.invalidApiSpecErrorName = exports.specParserGenerateResultWarningsTelemetryProperty = exports.specParserGenerateResultAllSuccessTelemetryProperty = exports.specParserGenerateResultTelemetryEvent = exports.getParserOptions = void 0;
...
var openApiParserOptions_2 = require("../../../common/openApiParserOptions");
Object.defineProperty(exports, "getParserOptions", { enumerable: true, get: function () { return openApiParserOptions_2.getParserOptions; } });
...
async function listOperations(context, apiSpecUrl, inputs, includeExistingAPIs = true, shouldLogWarning = true, existingCorrelationId) {
    ...
    try {
        const specParser = new m365_spec_parser_1.SpecParser(apiSpecUrl, (0, openApiParserOptions_1.getParserOptions)(projectType, undefined, inputs.platform));
        let validationRes;
        ...
        const multipleAuthAPIs = listResult.APIs.filter((api) => api.auth && api.auth.authScheme?.type === "multipleAuth");
...
async function generateScaffoldingSummary(warnings, teamsManifest, apiSpecFilePath, pluginManifestPath, projectPath) {
    ...
    const mcpWarningMessage = warnings.filter(mcpAuthScaffolder_1.isMCPScaffoldWarning).map((warn) => {
        return `${constant_1.SummaryConstant.NotExecuted} ${warn.content}`;
    });
    if (apiSpecWarningMessage.length ||
        ...
        if (mcpWarningMessage.length) {
            details += os_1.EOL + mcpWarningMessage.join(os_1.EOL);
        }
        ...
        if (optionalParamsOnlyWarnings) {
            for (const optionalParamsOnlyWarning of optionalParamsOnlyWarnings) {
                resultWarnings.push((0, localizeUtils_1.getLocalizedString)("core.copilotPlugin.scaffold.summary.warning.api.optionalParametersOnly", optionalParamsOnlyWarning.data.commandId, optionalParamsOnlyWarning.data.commandId) +
                    (0, localizeUtils_1.getLocalizedString)("core.copilotPlugin.scaffold.summary.warning.api.optionalParametersOnly.mitigation", optionalParamsOnlyWarning.data.parameterName, optionalParamsOnlyWarning.data.commandId, path_1.default.join(teamsfx_api_1.AppPackageFolderName, teamsfx_api_1.ManifestTemplateFileName), path_1.default.join(teamsfx_api_1.AppPackageFolderName, teamsManifest.composeExtensions[0].apiSpecificationFile ?? "")));
            }
        }
        ...
        for (const command of commands) {
            if (command.type === "query") {
                if (!command.apiResponseRenderingTemplateFile) {
                    const errorDetail = warnings.find((w) => w.type === m365_spec_parser_1.WarningType.GenerateCardFailed && w.data === command.id)?.content;
...
async function updateActionForCustomApi(specItems, language, chatFolder) {
    if (commonLanguages.includes(language)) {
        const actionsFilePath = path_1.default.join(chatFolder, "functions.json");
        const actions = [];
        for (const item of specItems) {
            const parameters = {
                type: "object",
...
async function updateCodeForCustomApi(specItems, language, destinationPath, openapiSpecFileName) {
    if (language === constants_2.ProgrammingLanguage.JS || language === constants_2.ProgrammingLanguage.TS) {
        ...
        const appFolderPath = path_1.default.join(destinationPath, "src");
        const functionDefinitionTemplate = functionDefinitionCode["python"];
        const functionHandlerTemplate = functionHandlerCode["python"];
        const appFilePath = path_1.default.join(appFolderPath, "app.py");
        const handlerFilePath = path_1.default.join(appFolderPath, "handlers.py");
        const functionDefinitionsCode = [];
        const functionHandlersCode = [];
        const operationIds = [];
        for (const item of specItems) {
        ...
        const updatedAppFileContent = appFileContent
            .replace("// Replace with function definition code", `prompt${functionDefinitionsCode.join("")}`)
        ...
        for (const item of specItems) {
            functionDefinitionsCode.push(functionDefinitionTemplate
                .replace(/{{operationId}}/g, item.item.operationId)
        ...
        const controllerFilePath = path_1.default.join(destinationPath, "Controllers", "Controller.cs");
        const controllerFileContent = (await fs_extra_1.default.readFile(controllerFilePath)).toString();
        const updatedControllerFileContent = controllerFileContent.replace("// Replace with function definition code", `${functionDefinitionsCode.join("\n")};`);
        await fs_extra_1.default.writeFile(controllerFilePath, updatedControllerFileContent);
        const handlerFilePath = path_1.default.join(destinationPath, "Functions", "Handlers.cs");
        const handlerFileContent = (await fs_extra_1.default.readFile(handlerFilePath)).toString();
        const updatedHandlerFileContent = handlerFileContent.replace("// Replace with function handler code", `${functionHandlersCode.join("\n")}`);
        await fs_extra_1.default.writeFile(handlerFilePath, updatedHandlerFileContent);
        const startFilePath = path_1.default.join(destinationPath, "Program.cs");
        const startFileContent = (await fs_extra_1.default.readFile(startFilePath)).toString();
        const updatedStartFileContent = startFileContent.replace("{{OPENAPI_SPEC_PATH}}", "apiSpecificationFile/" + openapiSpecFileName);
        await fs_extra_1.default.writeFile(startFilePath, updatedStartFileContent);
...
async function updateForCustomApi(spec, language, destinationPath, openapiSpecFileName) {
    const warnings = [];
    let chatFolder = path_1.default.join(destinationPath, "src", "app");
    if (language === constants_2.ProgrammingLanguage.CSharp) {
        chatFolder = path_1.default.join(destinationPath, "Functions");
    }
    else if (language === constants_2.ProgrammingLanguage.PY) {
        chatFolder = path_1.default.join(destinationPath, "src");
    }
...
async function generateAdaptiveCardInPluginManifestForKiota(pluginManifestPath, specPath, context) {
    try {
        ...
        const specParser = new m365_spec_parser_1.SpecParser(specPath, (0, openApiParserOptions_1.getParserOptions)(m365_spec_parser_1.ProjectType.Copilot, true));

...
exports.Lifecycle = exports.allowedEmptyKeys = void 0;
...
exports.allowedEmptyKeys = ["APP_NAME_SUFFIX"];
function resolveString(val, resolved, unresolved) {
    ...
    while (matches != null) {
        ...
        if (exports.allowedEmptyKeys.includes(envVar)) {
        ...
        else if (constants_1.OpenAIEnvironmentVariables.hasOwnProperty(envVar) ||
            constants_1.AzureEnvironmentVariables.hasOwnProperty(envVar)) {
            if (envVal) {
                resolved.push(envVar);
                newVal = newVal.replace(matches[0], envVal);
            }
    ...
    async execute(ctx) {
        ...
        telemetryReporter.sendEndEvent({
            ...
            properties: {
                ...
                [constant_1.TelemetryProperty.FailedAction]: (0, lodash_1.camelCase)(failedAction) ?? "",
            },
        }, e);
    ...
    async executeImpl(ctx, resolved, unresolved) {
        ...
        for (const driver of drivers) {
            ...
            if (result.isErr()) {
                ...
                summary.push(`${constant_1.SummaryConstant.Failed} ${errorMessage}`);
                return {
                    result: (0, teamsfx_api_1.err)({
                        kind: "PartialSuccess",
                        env: envOutput,
            ...
            if (result.value.size > 0 && ctx.projectPath && process.env.TEAMSFX_ENV) {
                const writeRes = await envUtil_1.envUtil.writeEnv(ctx.projectPath, process.env.TEAMSFX_ENV, envUtil_1.envUtil.map2object(envOutput));
                if (writeRes.isErr()) {
                    ctx.logProvider.warning(`Failed to flush env output to disk after action ${this.stringifyDriverDef(driver)}: ${writeRes.error?.message ?? String(writeRes.error)}`);
                }
            }

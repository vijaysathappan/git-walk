...
class Coordinator {
    async create(context, inputs, actionContext) {
        ...
        if (scratch === constants_2.ScratchOptions.no().id) {
            ...
            if (projectType === ProjectTypeOptions_1.ProjectTypeOptions.customEngineAgentOptionId ||
                ...
                (0, lodash_1.merge)(actionContext?.telemetryProps, {
                    [telemetry_1.TelemetryProperty.CustomCopilotRAG]: inputs["custom-copilot-rag"] ?? "",
                    [telemetry_1.TelemetryProperty.CustomCopilotAgent]: inputs["custom-copilot-agent"] ?? "",
                    [telemetry_1.TelemetryProperty.LlmService]: inputs["llm-service"] ?? "",
    ...
    async ensureTeamsFxInCsproj(projectPath) {
        ...
        const existItems = ItemGroup.filter((item) => {
            if (item.ProjectCapability && item.ProjectCapability[0])
                if (item.ProjectCapability[0]["$"]?.Include === "TeamsFx")
                    return true;
            return false;
        });
    ...
    async provision(ctx, inputs) {
        ...
        let hasError = false;
        try {
            ctx.progressBar = ctx.ui?.createProgressBar((0, localizeUtils_1.getLocalizedString)("core.progress.provision"), steps);
        ...
        if (projectModel.deploy) {
            ...
            try {
                ...
                if (inputs.platform !== teamsfx_api_1.Platform.CLI) {
                    ctx.progressBar = ctx.ui?.createProgressBar((0, localizeUtils_1.getLocalizedString)("core.progress.deploy"), steps);
                    await ctx.progressBar?.start();
                }
                const maybeDescription = summaryReporter.getLifecycleDescriptions();
                if (maybeDescription.isErr()) {
    ...
    async publish(ctx, inputs) {
        ...
        if (projectModel.publish) {
            ...
            try {
                ...
                ctx.progressBar = ctx.ui?.createProgressBar((0, localizeUtils_1.getLocalizedString)("core.progress.publish"), steps);
                await ctx.progressBar?.start();
                const maybeDescription = summaryReporter.getLifecycleDescriptions();
                if (maybeDescription.isErr()) {
                    hasError = true;
                ...
                if (result[1]) {
                    ...
                    const isCopilotAgentPublish = projectModel.publish.driverDefs.some((def) => def.uses === "copilotAgent/publish");
                    const portalUrl = isCopilotAgentPublish
                        ? constants_4.Constants.MICROSOFT_ADMIN_CENTER
                        : constants_4.Constants.TEAMS_ADMIN_PORTAL;
                    if (ctx.platform !== teamsfx_api_1.Platform.CLI) {
...
function showAadResourceLink(ctx, isAadUpdateAction, projectModel, clientId) {
    ...
    if (isAadUpdateAction &&
        ...
        ctx.ui?.showMessage("info", aadMsg, false, aadTitle).then((result) => {
            const userSelected = result.isOk() ? result.value : undefined;
...
function downloadSampleHook(sampleId, sampleAppPath) {
    ...
    if (sampleId === "todo-list-SPFx") {
        ...
        void (0, glob_1.glob)(`${sampleAppPath}/**/*.json`, { nodir: true, dot: true }).then((files) => Promise.all(files.map(async (file) => {

...
class FuncToolChecker {
    ...
    async resolve(installOptions) {
        ...
        try {
            ...
            return await this.getDepsInfo(undefined, undefined, new error_1.DepsCheckerError(error.message, helpLink_1.v3DefaultHelpLink));
        }
    }
    async getInstallationInfo(installOptions) {
        ...
        if (portableFunc) {
            ...
            if (symlinkDir) {
                await (0, fileHelper_1.createSymlink)(portableFunc.binFolder, symlinkDir);
    ...
    checkExpectedFuncAndNode(funcVersion, nodeVersion) {
        ...
        if (funcVersionRange && !semver_1.default.satisfies(funcVersion, funcVersionRange)) {
            return new error_1.DepsCheckerError(message_1.Messages.portableFuncNodeNotMatched(nodeVersion, funcVersion), helpLink_1.v3DefaultHelpLink);
        }
        return undefined;
    }
    ...
    async checkFuncVersion(expectedFuncVersion, binFolder, sentinelPath) {
        try {
            ...
            if (!hasSentinel) {
                return (0, teamsfx_api_1.err)(new error_1.DepsCheckerError(message_1.Messages.noSentinelFile(), helpLink_1.v3DefaultHelpLink));
            }
            const funcVersionSupport = isFuncVersionSupport(funcVersion, expectedFuncVersion);
            if (!funcVersionSupport) {
                return (0, teamsfx_api_1.err)(new error_1.DepsCheckerError(message_1.Messages.funcVersionNotMatch(funcVersion.versionStr, expectedFuncVersion), helpLink_1.v3DefaultHelpLink));
            }
            return (0, teamsfx_api_1.ok)(funcVersion);
        }
        catch (error) {
    ...
    async install(expectedFuncVersion, symlinkDir) {
        if ((0, system_1.isLinux)()) {
            throw new error_1.InstallSoftwareError(funcToolName, helpLink_1.functionToolInstallationLink);
        }
        if (!(await this.hasNPM())) {
            throw new error_1.DepsCheckerError(message_1.Messages.needInstallNpm(), helpLink_1.functionToolInstallationLink);
        }
        ...
        if (funcVersionRes.isErr()) {
            ...
            throw new error_1.DepsCheckerError(message_1.Messages.failToValidateFuncCoreTool(funcVersionRes.error.message), helpLink_1.functionToolInstallationLink);
        }
        this.telemetryProperties[telemetry_1.TelemetryProperties.InstalledFuncVersion] =
            funcVersionRes.value.versionStr;
        await (0, fileHelper_1.rename)(FuncToolChecker.getFuncInstallPath(tmpVersion), FuncToolChecker.getFuncInstallPath(funcVersionRes.value.versionStr));
        const binFolder = FuncToolChecker.getPortableFuncBinFolder(funcVersionRes.value.versionStr);
        if (symlinkDir) {
            await (0, fileHelper_1.createSymlink)(binFolder, symlinkDir);
    ...
    static getDefaultInstallPath() {
        return path.join(homedir(), `.${teamsfx_api_1.ConfigFolderName}`, "bin", "azfunc");
    }
    static getFuncInstallPath(versionStr) {
        return versionStr
            ? path.join(this.getDefaultInstallPath(), versionStr)
            : path.join(homedir(), `.${teamsfx_api_1.ConfigFolderName}`, "bin", "func");
    }
    ...
    static getHistorySentinelPath() {
        return path.join(homedir(), `.${teamsfx_api_1.ConfigFolderName}`, "func-sentinel");
    }
    ...
    async queryFuncVersion(funcBinFolder) {
        ...
        { shell: (0, system_1.isWindows)() ? "cmd.exe" : true }, `"${execPath}"`, "--version");
    ...
    async installFunc(tmpVersion, expectedFuncVersion) {
        try {
            ...
            await cpUtils_1.cpUtils.executeCommand(undefined, undefined, { timeout: timeout, shell: (0, system_1.isWindows)() ? "cmd.exe" : true }, this.getExecCommand("npm"), "install", 
            ...
            throw new error_1.DepsCheckerError({
                default: (0, localizeUtils_1.getDefaultString)("error.common.InstallSoftwareError", funcToolName),
                localized: (0, localizeUtils_1.getLocalizedString)("error.common.InstallSoftwareError", funcToolName),
            }, helpLink_1.functionToolInstallationLink);
        }
    }
    getExecCommand(command) {
        return (0, system_1.isWindows)() ? `${command}.cmd` : command;
    }
}
...
function mapToFuncToolsVersion(output) {
    ...
    if (!match) {
        throw new error_1.DepsCheckerError(message_1.Messages.invalidFuncVersion(output), helpLink_1.v3DefaultHelpLink);
    }

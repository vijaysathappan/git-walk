...
class EnvUtil {
    ...
    async readEnv(projectPath, env, loadToProcessEnv = true, silent = true) {
        ...
        if (dotEnvFilePathRes.isErr())
            return (0, teamsfx_api_1.err)(dotEnvFilePathRes.error);
        ...
        if (!dotEnvFilePath || !(await fs_extra_1.default.pathExists(dotEnvFilePath))) {
            if (silent) {
                ...
                return (0, teamsfx_api_1.ok)({ TEAMSFX_ENV: env });
            }
            else {
                return (0, teamsfx_api_1.err)(new common_1.FileNotFoundError("core", dotEnvFilePath || `.env.${env}`));
            }
        }
        // deserialize
            ...
            for (const key of Object.keys(parseResultSecret.obj)) {
                if (key.startsWith("SECRET_")) {
                    ...
                    if (raw.startsWith("crypto_")) {
                        ...
                        if (decryptRes.isErr())
                            return (0, teamsfx_api_1.err)(decryptRes.error);
    ...
    async writeEnv(projectPath, env, envs) {
        ...
        const parsedDotenvSecret = envSecretFileExists
            ...
            : { obj: {} };
        (0, lodash_1.merge)(parsedDotenv.obj, noneSecretEnv);
        (0, lodash_1.merge)(parsedDotenvSecret.obj, secretEnv);
        //serialize
    ...
    async listEnv(projectPath, remoteOnly = false) {
        ...
        return (0, teamsfx_api_1.ok)(envs);
    }
    object2map(obj) {
        const map = new Map();
    ...
    async resetEnv(projectPath, env, ignoreKeys) {
        const envFilePathRes = await pathUtils_1.pathUtils.getEnvFilePath(projectPath, env);
        if (envFilePathRes.isErr())
            return;
    ...
    async resetEnvFile(envFilePath, ignoreKeys) {
        const parseResult = exports.dotenvUtil.deserialize(await fs_extra_1.default.readFile(envFilePath, { encoding: "utf8" }));
        for (const key of Object.keys(parseResult.obj)) {
            if (!ignoreKeys.includes(key)) {
                parseResult.obj[key] = "";
            }
        }
        await fs_extra_1.default.writeFile(envFilePath, exports.dotenvUtil.serialize(parseResult), { encoding: "utf8" });
    }
}
...
class DotenvUtil {
    ...
    serialize(parsed) {
        const array = [];
        const obj = (0, lodash_1.cloneDeep)(parsed.obj);
        //append lines
        if (parsed.lines) {

...
exports.LocalCertificateManager = exports.localCertificateManagerDeps = void 0;
...
exports.localCertificateManagerDeps = {
    ...
    localize: localizeUtils_1.getLocalizedString,
};
const installText = () => exports.localCertificateManagerDeps.localize("debug.install");
const learnMoreText = () => exports.localCertificateManagerDeps.localize("core.provision.learnMore");
// TODO(xiaofhua): update help link for v3
const learnMoreUrl = "https://aka.ms/teamsfx-ca-certificate";
const warningMessage = () => exports.localCertificateManagerDeps.localize("debug.warningMessage");
const confirmMessage = () => warningMessage() + exports.localCertificateManagerDeps.localize("debug.warningMessage2");
const trustCertificateCancelError = () => new teamsfx_api_1.UserError({
    ...
    message: (0, localizeUtils_1.getDefaultString)("error.TrustCertificateCancelError"),
    displayMessage: exports.localCertificateManagerDeps.localize("error.TrustCertificateCancelError"),
});
class LocalCertificateManager {
    ...
    constructor(ui, logger) {
        ...
        this.certFolder = path
            .normalize(`${exports.localCertificateManagerDeps.homedir()}/.${teamsfx_api_1.ConfigFolderName}/certificate`)
            .split(path.sep)
            .join(path.posix.sep);
    }
    /**
    ...
    async setupCertificate(needTrust, checkOnly = false) {
        ...
        try {
            ...
            await exports.localCertificateManagerDeps.ensureDir(this.certFolder);
            if ((await exports.localCertificateManagerDeps.pathExists(certFilePath)) &&
                (await exports.localCertificateManagerDeps.pathExists(keyFilePath))) {
                const certContent = await exports.localCertificateManagerDeps.readFile(certFilePath, {
                    encoding: "utf8",
                });
                const keyContent = await exports.localCertificateManagerDeps.readFile(keyFilePath, {
                    encoding: "utf8",
                });
    ...
    async generateCertificate(certFile, keyFile) {
        ...
        const serialNumber = (0, uuid_1.v4)().replace(/-/g, "");
        const attrs = [
            {
                name: "commonName",
                value: "localhost",
        ...
        await exports.localCertificateManagerDeps.writeFile(certFile, certContent, { encoding: "utf8" });
        await exports.localCertificateManagerDeps.writeFile(keyFile, keyContent, { encoding: "utf8" });
        return thumbprint;
    }
    ...
    async verifyCertificateInStore(thumbprint) {
        try {
            if (exports.localCertificateManagerDeps.osType() === "Windows_NT") {
                return await this.checkCertificateWindows(thumbprint);
            }
            else if (exports.localCertificateManagerDeps.osType() === "Darwin") {
                const listCertCommand = `security find-certificate -c localhost -a -Z -p "${exports.localCertificateManagerDeps.homedir()}/Library/Keychains/login.keychain-db"`;
                const existingCertificates = await exports.localCertificateManagerDeps.execShell(listCertCommand);
    ...
    async trustCertificate(localCert, thumbprint, friendlyName) {
        try {
            if (exports.localCertificateManagerDeps.osType() === "Windows_NT") {
                ...
                await exports.localCertificateManagerDeps.execShell(`security add-trusted-cert -p ssl -k "${exports.localCertificateManagerDeps.homedir()}/Library/Keychains/login.keychain-db" "${localCert.certPath}"`);
                localCert.isTrusted = true;
                return;
            }
            else {
    ...
    async checkCertificateWindows(thumbprint) {
        const quotedThumbprint = shellQuote.quote([thumbprint]);
        try {
            // try powershell first
            const getCertCommand = `Get-ChildItem -Path Cert:\\CurrentUser\\Root | Where-Object { $_.Thumbprint -match '${quotedThumbprint}' }`;
            const getCertRes = await exports.localCertificateManagerDeps.execPowerShell(getCertCommand);
            ...
            const getCertRes = (await exports.localCertificateManagerDeps.execShell(getCertCommand)).trim();
            return getCertRes.toUpperCase().includes(thumbprint.toUpperCase());
        }
    }
    async trustCertificateWindows(localCert, thumbprint, friendlyName) {
        try {
            ...
            await exports.localCertificateManagerDeps.execPowerShell(installCertCommand);
            try {
                const friendlyNameCommand = `(Get-ChildItem -Path Cert:\\CurrentUser\\Root\\${thumbprint}).FriendlyName='${friendlyName}'`;
                await exports.localCertificateManagerDeps.execPowerShell(friendlyNameCommand);
            }
            catch (e) {
            ...
            await exports.localCertificateManagerDeps.execShell(installCertCommand);
            try {
                const certInfPath = path.join(path.dirname(localCert.certPath), "localhost.inf");
                await exports.localCertificateManagerDeps.writeFile(certInfPath, [
                    "[Version]",
                ...
                await exports.localCertificateManagerDeps.execShell(friendlyNameCommand);
            }
            catch (e) {
                // ignore friendly name failure
            }

...
// Returns the distro-specific subfolder under dist/linux/ for the current system.
function getLinuxDistroSubdir() {
    try {
        var osRelease = require("fs").readFileSync("/etc/os-release", "utf8");
        var versionMatch = osRelease.match(/^VERSION_ID="?(\d+)/m);
        var majorVersion = versionMatch ? parseInt(versionMatch[1], 10) : 0;
        if (/^ID(?:_LIKE)?=.*\brhel\b/m.test(osRelease)) {
            return majorVersion >= 10 ? "rhel10" : "rhel8_9";
        }
    }
    catch (_a) {
        // Fall through to the default distro directory.
    }
    return "ubuntu";
}
// Resolves the native addon directly from its platform/arch-specific folder under dist/.
// The dependent shared library (msalruntime.dll / .dylib / .so) lives alongside the .node
// in the same folder and is found via LOAD_WITH_ALTERED_SEARCH_PATH (Windows) or the
// @loader_path / $ORIGIN rpath baked in at build time (macOS / Linux).
function getNativeBinaryPath() {
    ...
    var platform = platformDir[process.platform];
    var arch = process.arch === "ia32" ? "x86" : process.arch;
    var subdir = process.platform === "linux"
        ? path.join("linux", getLinuxDistroSubdir(), arch)
        : path.join(platform, arch);
    return path.join(__dirname, subdir, "msal-node-runtime.node");
}
var msalNodeRuntime;
try {
    msalNodeRuntime = require(getNativeBinaryPath());
}
catch (_a) {

 ...
 * @param addOidcScopes
 */
function addScopes(parameters, scopes, correlationId, addOidcScopes = true, defaultScopes = OIDC_DEFAULT_SCOPES$1) {
...
function addClaims(parameters, correlationId, claims, clientCapabilities, skipBrokerClaims, claimsToMerge) {
    ...
    const configClaims = skipBrokerClaims && parameters.has(BROKER_CLIENT_ID)
        ? undefined
        : clientCapabilities;
    const mergedClaims = buildMergedClaims$1(claims, configClaims, correlationId, claimsToMerge);
 ...
 */
function buildMergedClaims$1(claims, clientCapabilities, correlationId = "", claimsToMerge) {
...
class CacheManager {
    ...
    getAccessToken(account, request, tokenKeys, targetRealm) {
        ...
        const accessTokenFilter = {
            ...
            keyId: authScheme === AuthenticationScheme$1.SSH
                ? request.sshKid
                : undefined,
 ...
 * @internal
 */
function buildClientConfiguration({ authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, tokenBindingKeyManager: tokenBindingKeyManager, clientCredentials: clientCredentials, libraryInfo: libraryInfo, telemetry: telemetry, serverTelemetryManager: serverTelemetryManager, persistencePlugin: persistencePlugin, serializableCache: serializableCache, }) {
    const loggerOptions = {
...
class ResponseHandler {
    ...
    async handleServerTokenResponse(serverTokenResponse, authority, reqTimestamp, request, apiId, authCodePayload, userAssertionHash, handlingRefreshTokenResponse, forceCacheRefreshTokenResponse, serverRequestId, additionalCacheKeyComponents) {
        ...
        try {
            ...
            if (handlingRefreshTokenResponse &&
                ...
                if (cachedAccounts.length < 1) {
                    ...
                    return await ResponseHandler.generateAuthenticationResult(this.cryptoObj, authority, cacheRecord, false, request, this.performanceClient, {
                        idTokenClaims,
        ...
        return ResponseHandler.generateAuthenticationResult(this.cryptoObj, authority, cacheRecord, false, request, this.performanceClient, {
            idTokenClaims,
            requestState: requestStateObj,
            serverTokenResponse,
            requestId: serverRequestId,
...
const ECDSA_P256_KEYGEN_ALGORITHM_OPTIONS = {
    name: "ECDSA",
    namedCurve: JSON_WEB_KEY_CURVE_P256,
};
const ECDSA_SHA256_SIGN_ALGORITHM_OPTIONS = {
    name: "ECDSA",
...
const buildMergedClaims = buildMergedClaims$1;
 ...
 * Licensed under the MIT License.
 */
const SUPPORTED_AUTHENTICATION_SCHEMES = new Set([
    AuthenticationScheme$1.BEARER,
    AuthenticationScheme$1.POP,
...
async function initializeBaseRequest(request, config, performanceClient, logger, correlationId) {
    ...
    if (!validatedRequest.authenticationScheme) {
        ...
        if (!SUPPORTED_AUTHENTICATION_SCHEMES.has(validatedRequest.authenticationScheme)) {
            throw createClientConfigurationError(unsupportedAuthenticationScheme, correlationId);
        }
        if (validatedRequest.authenticationScheme ===
            AuthenticationScheme$1.SSH) {
            if (!request.sshJwk) {
                throw createClientConfigurationError(missingSshJwk, correlationId);
            }
            if (!request.sshKid) {
                throw createClientConfigurationError(missingSshKid, correlationId);
            }
        }
...
async function waitForPopupRelayResponse(timeoutMs, logger, request, popupWindow, performanceClient) {
    return new Promise((resolve, reject) => {
        ...
        const { libraryState } = parseRequestState(base64Decode, request.state || "", correlationId);
...
class PlatformAuthInteractionClient extends BaseInteractionClient {
    ...
    async acquireTokenRedirect(request, rootMeasurement, options) {
        ...
        this.browserStorage.setTemporaryCache(TemporaryCacheKeys.NATIVE_REQUEST, JSON.stringify({
            ...nativeRequest,
    ...
    async handleRedirectPromise() {
         ...
         */
        const { prompt, storeInCache, ...request } = cachedRequest;
        if (prompt) {
    ...
    async cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId, reqTimestamp, environment, storeInCache) {
        ...
        return this.nativeStorageManager.saveCacheRecord(nativeCacheRecord, this.correlationId, isKmsi(idTokenClaims), this.apiId, storeInCache);
    }
    ...
    async initializePlatformRequest(request) {
        ...
        const mergedClaims = buildMergedClaims$1(claims, configClaims?.length ? configClaims : undefined);
    ...
    handleExtraBrokerParams(request, embeddedClientId) {
        ...
        const { 
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        [BROKER_CLIENT_ID]: _brkClientId, 
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        [BROKER_REDIRECT_URI]: _brkRedirectUri, 
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        [CLIENT_ID]: _clientId, ...remainingExtraParameters } = request.extraParameters || {};
        request.extraParameters = {
...
async function getStandardParameters(config, authority, request, logger, performanceClient) {
    ...
    if (request.platformBroker) {
        ...
        if (request.authenticationScheme === AuthenticationScheme$1.POP) {
            ...
            const tokenBindingKeyManager = new TokenBindingKeyManager(logger, performanceClient);
...
async function getEARFormData(config, authority, request, logger, performanceClient) {
    ...
    const action = getAuthorizeUrl(authority, queryParams);
        ...
        for (let attempt = 1; attempt <= MAX_FETCH_POST_RETRIES + 1; attempt++) {
            try {
                ...
                if (!shouldRetryPostFetchError(lastError, attempt)) {
                    throw createNetworkError(createBrowserAuthError(window.navigator.onLine !== false
                        ? postRequestFailed
                        : noNetworkConnectivity, correlationId || ""), undefined, undefined, lastError);
                }
...
function shouldRetryPostFetchError(error, attempt) {
    return (attempt <= MAX_FETCH_POST_RETRIES &&
...
class PlatformAuthDOMHandler {
    ...
    initializePlatformDOMRequest(request) {
        ...
        const { accountId, clientId, authority, scope, redirectUri, correlationId, state, extraParameters, ...remainingProperties } = request;
...
class PopupClient extends StandardInteractionClient {
        ...
        try {
            ...
            if (popupRequest.httpMethod === HttpMethod.POST) {
                ...
                const popupNavigateUrl = popupRelayUri
                    ? buildPopupRelayUrl(popupRelayUri, this.getRelayStateId(request.state, correlationId), { method: "GET", url: navigateUrl }, correlationId)
                    : navigateUrl;
    ...
    async logoutPopupAsync(validRequest, popupParams, requestAuthority, mainWindowRedirectUri) {
        ...
        try {
            ...
            const popupRelayUri = this.config.auth.popupRelayUri;
            const popupNavigateUrl = popupRelayUri
                ? buildPopupRelayUrl(popupRelayUri, this.getRelayStateId(validRequest.state || "", this.correlationId), { method: "GET", url: logoutUri }, this.correlationId)
                : logoutUri;
    ...
    getRelayStateId(state, correlationId) {
        return parseRequestState(base64Decode, state, correlationId).libraryState.id;
    }
    /**
...
class RedirectClient extends StandardInteractionClient {
    ...
    getRedirectResponse(userProvidedResponse) {
        ...
        if (response) {
            try {
                validateInteractionType(response, this.browserCrypto, exports.InteractionType.Redirect, this.correlationId);
            }
            catch (e) {
                if (e instanceof AuthError) {
    ...
    async acquireToken(request) {
        ...
        if (inputRequest.prompt) {
            if (inputRequest.prompt !== PromptValue$1.NONE &&
                ...
                this.logger.warning(`0bmctg ${inputRequest.prompt} ${PromptValue$1.NONE}`, this.correlationId);
    ...
    async handleRedirectPromiseInternal(options) {
        ...
        try {
            if (useNative && this.platformAuthProvider) {
                ...
                // Cached nativeAccountId routes directly to the broker (no /authorize call)
                rootMeasurement.add({
...
function toAuthError(err) {
    ...
    switch (err.code) {
        ...
        case WebBrokerBridgeErrorCode.UserCanceled:
            return new ClientAuthError(userCanceled, correlationId);
        case WebBrokerBridgeErrorCode.NoNetwork:
            return new ClientAuthError(noNetworkConnectivity$1, correlationId);
        case WebBrokerBridgeErrorCode.AccountUnavailable:
            return new ClientAuthError(noAccountFound, correlationId);
        case WebBrokerBridgeErrorCode.BridgeDisabled:
            return new ClientAuthError(nestedAppAuthBridgeDisabled, correlationId);
        case WebBrokerBridgeErrorCode.BridgeUnavailable:
             ...
             */
            return new ClientAuthError(err.innerErrorCode ||
                nestedAppAuthBridgeDisabled, correlationId, err.description);
        /*
...
class NestedAppAuthAdapter {
    ...
    toNaaTokenRequest(request) {
        ...
        const claims = buildMergedClaims$1(request.claims, this.clientCapabilities);
        const scopes = request.scopes || OIDC_DEFAULT_SCOPES$1;
        const attributeTokens = serializeAttributeTokens(request.attributeTokens);
        const tokenRequest = {
    ...
    fromNaaTokenResponse(request, response, reqTimestamp) {
        if (!response.token.id_token || !response.token.access_token) {
            throw createClientAuthError(nullOrEmptyToken, request.correlationId);
        }
        ...
        const idTokenClaims = extractTokenClaims(response.token.id_token, this.crypto.base64Decode, request.correlationId);
    ...
    toAuthenticationResultFromCache(account, idToken, accessToken, request, correlationId) {
        ...
        const idTokenClaims = extractTokenClaims(idToken.secret, this.crypto.base64Decode, correlationId);
...
class BridgeProxy {
        ...
        try {
            window.nestedAppAuthBridge.addEventListener("message", (response) => {
                ...
                if (responseEnvelope.success) {
                    BridgeProxy.pendingRegistry.resolve(responseEnvelope.requestId, responseEnvelope);
                }
                else {
                    BridgeProxy.pendingRegistry.reject(responseEnvelope.requestId, responseEnvelope.error);
                }
            });
            const bridgeResponse = await BridgeProxy.sendRequestViaRegistry(BridgeProxy.buildRequest("GetInitContext"));
    ...
    sendRequest(method, requestParams) {
        return BridgeProxy.sendRequestViaRegistry(BridgeProxy.buildRequest(method, requestParams));
    }
    /**
...
async function loadExternalTokens(config, request, response, options, performanceClient = new StubPerformanceClient()) {
    ...
    try {
        const idTokenClaims = response.id_token
            ? extractTokenClaims(response.id_token, base64Decode, correlationId)
            : undefined;
        ...
        const logger = new Logger(browserConfig.system.loggerOptions || {}, name, version);
...
async function loadAccessToken(request, response, homeAccountId, environment, tenantId, kmsi, options, correlationId, storage, logger, clientId) {
    ...
    const extendedExpiresOn = options.extendedExpiresOn ||
        (response.ext_expires_in || response.expires_in) +
            nowSeconds();
    const accessTokenEntity = createAccessTokenEntity(homeAccountId, environment, response.access_token, clientId, tenantId, scopes.printScopes(), expiresOn, extendedExpiresOn, base64Decode, correlationId);

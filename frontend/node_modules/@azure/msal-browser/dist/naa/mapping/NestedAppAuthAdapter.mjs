...
'use strict';
import { RequestParameterBuilder, CacheHelpers, StringUtils, Constants, createClientAuthError, ClientAuthErrorCodes, TimeUtils, AuthToken, getTenantIdFromIdTokenClaims, buildTenantProfile, AuthError } from '@azure/msal-common/browser';
import { isBridgeError } from '../BridgeError.mjs';
import { BridgeStatusCode } from '../BridgeStatusCode.mjs';
import { WebBrokerBridgeErrorCode } from '../../webBrokerBridge/WebBrokerBridgeError.mjs';
import { toAuthError } from '../../webBrokerBridge/WebBrokerBridgeErrorMap.mjs';
...
class NestedAppAuthAdapter {
    ...
    toNaaTokenRequest(request) {
        ...
        const claims = RequestParameterBuilder.buildMergedClaims(request.claims, this.clientCapabilities);
        const scopes = request.scopes || Constants.OIDC_DEFAULT_SCOPES;
        const attributeTokens = CacheHelpers.serializeAttributeTokens(request.attributeTokens);
        const tokenRequest = {
    ...
    fromNaaTokenResponse(request, response, reqTimestamp) {
        ...
        const idTokenClaims = AuthToken.extractTokenClaims(response.token.id_token, this.crypto.base64Decode, request.correlationId);
    ...
    toAuthenticationResultFromCache(account, idToken, accessToken, request, correlationId) {
        ...
        const idTokenClaims = AuthToken.extractTokenClaims(idToken.secret, this.crypto.base64Decode, correlationId);

...
'use strict';
import { ServerTelemetryManager, AADServerParamKeys, TimeUtils, AuthError, ScopeSet, createClientAuthError, ClientAuthErrorCodes, AuthToken, updateAccountTenantProfileData, UrlString, buildAccountToCache, AccountEntityUtils, AuthorityType, Constants, PopTokenGenerator, CacheHelpers, RequestParameterBuilder, invokeAsync, PerformanceEvents } from '@azure/msal-common/browser';
...
class PlatformAuthInteractionClient extends BaseInteractionClient {
    ...
    async acquireTokensFromCache(nativeAccountId, request) {
        ...
        try {
            ...
            const idTokenClaims = AuthToken.extractTokenClaims(idToken?.secret || "", base64Decode, this.correlationId);
    ...
    async handleNativeResponse(response, request, reqTimestamp, storeInCache) {
        ...
        const idTokenClaims = AuthToken.extractTokenClaims(response.id_token, base64Decode, this.correlationId);
    ...
    async cacheNativeTokens(response, request, homeAccountIdentifier, idTokenClaims, tenantId, reqTimestamp, environment, storeInCache) {
        ...
        const additionalCacheKeyComponents = request.attributeTokens
            ...
            : undefined;
        const cachedAccessToken = CacheHelpers.createAccessTokenEntity(homeAccountIdentifier, environment, response.access_token, request.clientId, idTokenClaims.tid || tenantId, responseScopes.printScopes(), tokenExpirationSeconds, 0, base64Decode, request.correlationId, undefined, request.tokenType, undefined, request.keyId, additionalCacheKeyComponents);
        ...
        return this.nativeStorageManager.saveCacheRecord(nativeCacheRecord, this.correlationId, AuthToken.isKmsi(idTokenClaims), this.apiId, storeInCache);
    }
    ...
    async initializePlatformRequest(request) {
        ...
        const mergedClaims = RequestParameterBuilder.buildMergedClaims(claims, configClaims?.length ? configClaims : undefined);
    ...
    handleExtraBrokerParams(request, embeddedClientId) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        [AADServerParamKeys.CLIENT_ID]: _clientId, ...remainingExtraParameters } = request.extraParameters || {};
        request.extraParameters = {

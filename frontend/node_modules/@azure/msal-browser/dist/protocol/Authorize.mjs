...
import { TokenBindingKeyManager } from '../crypto/TokenBindingKeyManager.mjs';
import { earJweEmpty, earJwkEmpty, nativeConnectionNotEstablished } from '../error/BrowserAuthErrorCodes.mjs';
...
async function getAuthCodeRequestUrl(config, authority, request, logger, performanceClient) {
    if (!request.codeChallenge) {
        throw createClientConfigurationError(ClientConfigurationErrorCodes.pkceParamsMissing, request.correlationId);
    }
...
async function handleResponsePlatformBroker(request, accountId, apiId, config, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
    ...
    const { userRequestState } = ProtocolUtils.parseRequestState(browserCrypto.base64Decode, request.state, request.correlationId);
...
async function handleResponseEAR(request, response, apiId, config, authority, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
    ...
    AuthorizeProtocol.validateAuthorizationResponse(response, request.state, request.correlationId);
    if (!response.ear_jwe) {
...
export { createForm, getAuthCodeRequestUrl, getCodeForm, getCodeFormData, getEARForm, getEARFormData, handleResponseCode, handleResponseEAR, handleResponsePlatformBroker, parseClientData };

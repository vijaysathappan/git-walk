...
class SignUpClient extends CustomAuthInteractionClientBase {
    ...
    async performChallengeRequest(request) {
        ...
        this.logger.error(`0wg2fe ${challengeResponse.challenge_type}`, challengeResponse.correlation_id || request.correlationId);
    ...
    async performContinueRequest(callerName, requestParams, telemetryManager, responseGetter, requestCorrelationId) {
        this.logger.verbose(`0sw5ym ${callerName}`, requestCorrelationId);
        try {
            const response = await responseGetter();
            this.logger.verbose(`0ib062 ${callerName}`, response.correlation_id || requestCorrelationId);
            ...
            if (error instanceof CustomAuthApiError) {
                ...
                this.logger.errorPii(`03rje8 ${callerName} ${error}`, requestCorrelationId);

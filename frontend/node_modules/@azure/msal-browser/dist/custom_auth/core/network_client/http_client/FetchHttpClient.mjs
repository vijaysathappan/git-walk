...
class FetchHttpClient {
    ...
    async sendAsync(url, options) {
        ...
        try {
            this.logger.verbosePii(`17dhbs ${url}`, correlationId);
            const startTime = performance.now();
            const response = await fetch(url, options);
            const endTime = performance.now();
            this.logger.verbosePii(`1egykc ${url} ${endTime - startTime} ${response.status}`, correlationId);
            return response;
        }
        catch (e) {
            this.logger.errorPii(`0ye3lm ${url} ${e}`, correlationId);

...
'use strict';
import { RequestParameterBuilder, UrlUtils, UrlString, ProtocolUtils } from '@azure/msal-common/browser';
...
async function waitForBridgeResponse(timeoutMs, logger, request, performanceClient, experimentalConfig) {
    return new Promise((resolve, reject) => {
        ...
        const { libraryState } = ProtocolUtils.parseRequestState(base64Decode, request.state || "", request.correlationId);
...
const buildMergedClaims = RequestParameterBuilder.buildMergedClaims;

export { blockAPICallsBeforeInitialize, blockAcquireTokenInPopups, blockNonBrowserEnvironment, blockRedirectInIframe, blockReloadInHiddenIframes, buildMergedClaims, cancelPendingBridgeResponse, clearAuthResponseFromUrl, clearHash, createGuid, getCurrentUri, getHomepage, isInIframe, isInPopup, parseAuthResponseFromUrl, preconnect, preflightCheck, redirectPreflightCheck, replaceHash, waitForBridgeResponse };

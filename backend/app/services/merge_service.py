...
from ..domain import RepositoryPolicy
...
class MergeService:
    def branch_divergence(
        ...
        if not source or not repository_role(source["repository_id"], actor.user_id):
    ...
    def list_requests(self, repository_id: str, actor: MergeActor) -> list[dict[str, Any]]:
        if not repository_role(repository_id, actor.user_id):
            raise PermissionError("Repository access is required")
    ...
    def get_request(self, merge_request_id: str, actor: MergeActor) -> dict[str, Any]:
        ...
        if not request or not repository_role(request["repository_id"], actor.user_id):
    ...
    def resolve_conflict(
        ...
        *,
        ...
        if repository_role(request["repository_id"], actor.user_id) not in {"owner", "editor"}:
            raise PermissionError("Editor access is required to resolve conflicts")
    ...
    def review(
        ...
        *,
        ...
        request = self.get_request(merge_request_id, actor)
        role = repository_role(request["repository_id"], actor.user_id)
        policy = RepositoryPolicy.from_value(role)
        if not policy or not policy.can_review:
            raise PermissionError("Only the repository owner can review merge requests")
    ...
    def merge(self, merge_request_id: str, actor: MergeActor) -> dict[str, Any]:
        ...
        role = repository_role(request["repository_id"], actor.user_id)
        policy = RepositoryPolicy.from_value(role)
        if not policy or not policy.can_merge:
            raise PermissionError("Only the repository owner can merge into main")
    ...
    def revert_commit(self, commit_id: str, actor: MergeActor) -> dict[str, Any]:
        ...
        role = repository_role(commit["repository_id"], actor.user_id)
        if not branch or branch["status"] != "ACTIVE" or role not in {"owner", "editor"}:

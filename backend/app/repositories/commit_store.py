...
def commit_semantic_delta(
    *,
    ...
    try:
        ...
        conn.execute("UPDATE BRANCHES SET HEAD_COMMIT_ID=?,UPDATED_AT=? WHERE BRANCH_ID=?", (commit_id, now, branch_id))
        structural = row_operations + column_operations + sheet_operations
        commit_number = conn.execute("SELECT COUNT(*) FROM COMMITS WHERE BRANCH_ID=?", (branch_id,)).fetchone()[0]
        checkpoint_created = commit_number % 25 == 0 or structural >= 100 or len(stored_changes) >= 2000
        if checkpoint_created:
            snapshot = semantic_snapshot(conn, table_id); snapshot["head_commit_id"] = commit_id
            conn.execute("INSERT INTO BRANCH_CHECKPOINTS VALUES (?, ?, ?, ?, ?, ?, ?)", (stable_id("CP"), repository_id, branch_id, commit_id, json.dumps(snapshot, default=str, separators=(",", ":")), "PERIODIC" if structural < 100 else "STRUCTURAL", now))
        conn.commit()
        return {
            ...
            "commit_hash": commit_hash, "timestamp": now,
        }
    except Exception:
        conn.rollback()
        raise
